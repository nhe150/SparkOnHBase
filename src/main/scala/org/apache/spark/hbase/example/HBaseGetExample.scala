...
object HBaseGetExample {
  def main(args: Array[String]) {
    if (args.length == 0) {
      System.out.println("HBaseGetExample {tableName}");
      return ;
    }

    val tableName = args(0);

    val sparkConf = new SparkConf().setAppName("HBaseGetExample " + tableName)
    ...
    val get = hbaseContext.Get[String](
      connection,  
      tableName, 
      Bytes.toBytes("1"),
      (result: Result) => {
